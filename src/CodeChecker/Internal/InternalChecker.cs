...
namespace CodeChecker
{
    ...
    /// </summary>
    internal class InternalChecker
    {
...
#pragma warning restore CS8777 // Le paramètre doit avoir une valeur non null au moment de la sortie.
        ...
        {
            Null(value, paramName, checkType, checkLevel);
            Fail(value.Length > 0, string.Format(ARGUMENT_NOTNULL_MSG_PATTERN, paramName), checkType, checkLevel, true);
            Fail(value[0] != '\0', string.Format(ARGUMENT_NOTNULL_MSG_PATTERN, paramName), checkType, checkLevel, true);
        }

        public static void NullOrEmpty<T>(ICollection<T>? values, CheckType checkType, CheckLevel checkLevel)
        {
            Null(values, "collection", checkType, checkLevel);
            Fail(values!.Any(), string.Format(ARGUMENT_NOTNULL_MSG_PATTERN, "collection"), checkType, checkLevel, true);
        }
