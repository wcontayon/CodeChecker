...
namespace CodeChecker.Model.validation.Attributes
{
    ...
    /// <summary>
    /// LessThan attribute, to specify that the property should have a value
    /// less than to a specific value.
    /// </summary>
    /// <remarks>Property should be in type short, int,long, decimal, float, ushort, uint, ulong/.</remarks>
    public class LessThanAttribute : BaseRuleAttribute
    {
        /// <summary>
        ...
        public override bool IsValid(object? item)
        {
            ...
            if (isValid)
            {
                isValid = InternalCheckerExtension.Compare(item!, this.Value!, Operator.LessThan);
            }
